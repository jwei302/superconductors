...
submit_eval_devel() {
    local beta=$1
    local jobid
    jobid=$(BETA="${beta}" sbatch dpo/scripts/eval_dpo_one.sbatch | awk '{print $4}')
...
ls -la artifacts/dpo/dpo_b0.5/dpo_final.ckpt 2>&1 || log "WARN: β=0.5 ckpt missing"

# ============================================================
# Phase 2: β=0.5 eval on gpu_devel
# ============================================================
log "phase 2: submit β=0.5 eval"
if [ -f artifacts/dpo/dpo_b0.5/dpo_final.ckpt ]; then
...
if [ "$state" = "PENDING" ]; then
    ...
    new_jobid=$(BETA=0.1 sbatch \
        ...
        --output=artifacts/dpo/dpo_devel_b0.1_%j.out \
        --error=artifacts/dpo/dpo_devel_b0.1_%j.err \
        dpo/scripts/train_dpo.sbatch | awk '{print $4}')
...
ls -la artifacts/dpo/dpo_b0.1/dpo_final.ckpt 2>&1 || log "WARN: β=0.1 ckpt missing"

# ============================================================
# Phase 4: β=0.1 eval on gpu_devel
# ============================================================
log "phase 4: submit β=0.1 eval"
if [ -f artifacts/dpo/dpo_b0.1/dpo_final.ckpt ]; then
    wait_devel_clear
...
baseline = json.loads(Path('artifacts/dpo/day1_baseline_pool/scores_y10k.json').read_text())
...
for beta in ['0.1','0.5','1','5','25']:
    p = Path(f'artifacts/dpo/eval/scores_dpo_b{beta}.json')
    if not p.exists():
