...
#SBATCH --mem=32G
#SBATCH --output=${PROJECT_DIR}/artifacts/dpo/dpo_devel_b${beta}_%j.out
#SBATCH --error=${PROJECT_DIR}/artifacts/dpo/dpo_devel_b${beta}_%j.err
...
python dpo/run_dpo.py \\
    --pairs_file ${PROJECT_DIR}/artifacts/dpo/pairs_v1.pt \\
    --model_path ${PROJECT_DIR}/models/superconductor_generator \\
    --out_dir ${PROJECT_DIR}/artifacts/dpo/dpo_b${beta} \\
    --beta ${beta} \\
    --max_steps 5000 \\
    --val_every 500 \\
    --batch_size 1 \\
...
ls -la artifacts/dpo/dpo_b*/dpo_final.ckpt 2>&1 || true
