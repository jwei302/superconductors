...
Outputs (all to artifacts/dpo/figures/):
...
OUT = Path("artifacts/dpo/figures")
...
baseline = load_pool("artifacts/dpo/day1_baseline_pool/scores_y10k.json")
sft8k    = load_pool("artifacts/dpo/eval/scores_sft_8k_seed1.json")
pools    = {b: load_pool(f"artifacts/dpo/eval/scores_dpo_b{b}.json") for b in BETAS}

T_25 = np.percentile(baseline, 25)
T_10 = np.percentile(baseline, 10)
T_05 = np.percentile(baseline, 5)

audit = json.loads(Path("artifacts/dpo/audit_summary.json").read_text())
...
def fig_training_components():
    ...
    for ax, b in zip(axes, BETAS):
        df = pd.read_csv(f"artifacts/dpo/dpo_b{b}/dpo/version_0/metrics.csv")
